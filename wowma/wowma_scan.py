...
port = 9228
cmd = "/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome --remote-debugging-port="+str(port)+" --no-first-run --incognito -default-browser-check --user-data-dir=$(mktemp -d -t 'chrome-remote_data_dir') --proxy-server=socks5://localhost:"+random.choice(p_port)
...
while True:
    ...
    while True:
        ...
        page = soup.find('p', 'resultText').text
        print(title + page)

        for img in soup.find_all('div',class_='image'):
